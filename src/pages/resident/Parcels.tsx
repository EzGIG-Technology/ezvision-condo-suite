        ...
        <Card key={p.id} className="flex flex-col gap-3 p-4">
          ...
          {tab === 'waiting' ? (
            <div className="flex items-center justify-between rounded-xl bg-ice p-3">
              ...
              <div className="text-right text-xs text-muted"><p>Shelf <b className="font-mono text-navy">{p.shelf}</b></p><p>Arrived {relative(p.loggedAt)}</p></div>
            </div>
          ) : <p className="text-xs text-muted">Collected by {p.collectedBy} · {when(p.collectedAt)}</p>}
          {tab === 'waiting' && <Button size="sm" variant="ghost" className="self-start" onClick={() => toast.success('Guard asked to hold it', 'Kept at the guardhouse for up to 7 days.')}>Hold for me, I'm away</Button>}
        </Card>
      ))}
