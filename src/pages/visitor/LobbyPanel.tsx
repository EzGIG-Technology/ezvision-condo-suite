...
export default function LobbyPanel() {
  ...
  const press = (d: string) => { setError(''); setDigits((x) => (x.length < 4 ? x + d : x)); };
  const dial = () => {
        ...
        {screen ? (
          ...
          <>
            <div className="flex flex-col items-center gap-1 rounded-3xl bg-night-panel p-6">
              ...
              <span className="font-mono text-4xl font-extrabold tracking-wider" aria-live="polite" aria-label={`Unit ${digits ? unit : 'not entered'}`}>{tower}-{(digits.slice(0, 2) || '··').padEnd(2, '·')}-{(digits.slice(2, 4) || '··').padEnd(2, '·')}</span>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <button type="button" aria-label="Delete digit" disabled={!digits} onClick={() => setDigits((x) => x.slice(0, -1))} className="flex h-16 items-center justify-center rounded-2xl text-muted-light hover:bg-night-panel disabled:opacity-40"><Delete className="h-6 w-6" /></button>
