...
import { Card, CardHeader, Chip, Drawer, Empty, Field, Input, Modal, Segmented, Select, Textarea } from '@/components/ui';
...
function LiftCalendar() {
  ...
  const [extra, setExtra] = useState<typeof bookings>([]);
  const [f, setF] = useState({ unit: 'A-15-07', what: 'Move-in', day: '1', slot: '09:00 to 12:00' });
  const all = [...bookings, ...extra];
  ...
  const submit = (e: FormEvent) => {
    ...
    setExtra([...extra, { id: `x-${extra.length}`, date: d.toISOString(), slot: f.slot, what: f.what, unit: f.unit, lift: `Tower ${f.unit[0]} service lift`, kind: f.what === 'Move-out' ? 'move_out' : f.what === 'Delivery' ? 'delivery' : 'move_in' }]);
    setOpen(false);
  ...
  return (
    <Card className="flex flex-col gap-4 p-5">
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4 lg:grid-cols-7">
        {days.map((d) => {
          const items = all.filter((b) => new Date(b.date).toDateString() === d.toDateString());
          ...
          return (
            <div key={d.toISOString()} className={cn('flex min-h-[140px] flex-col gap-2 rounded-xl p-2.5', today ? 'bg-brand-soft' : 'bg-[#F7F9FD]')}>
              ...
              {items.map((b) => (
                <div key={b.id} className="flex flex-col gap-0.5 rounded-lg bg-white p-2" style={{ boxShadow: `inset 3px 0 0 ${b.kind === 'move_out' ? '#F59E0B' : b.kind === 'delivery' ? '#14A38F' : '#1D4FE0'}` }}>
                  <span className="font-mono text-[11px] font-bold text-muted">{b.slot}</span><span className="text-xs font-bold">{b.what} · {b.unit}</span><span className="text-[11px] text-muted">{b.lift}</span>
                </div>
              ))}
            </div>
          );
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Book the service lift"
        ...
        <form id="lift" onSubmit={submit} className="grid gap-3 sm:grid-cols-2">
          ...
          <Field label="Slot">{(id) => <Select id={id} value={f.slot} onChange={(e) => setF({ ...f, slot: e.target.value })}><option>09:00 to 12:00</option><option>14:00 to 17:00</option></Select>}</Field>
        </form>
      </Modal>
    </Card>
  );
